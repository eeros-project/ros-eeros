...
DigOut::DigOut(std::string id,
  ...
  if ( msgType == "sensor_msgs::BatteryState" ) {
    batteryStatePublisher = rosNodeHandle->create_publisher<sensor_msgs::msg::BatteryState>(topic, queueSize);
...
void DigOut::sensorMsgsBatteryStatePresent(const DigOut& self, const bool value, const uint64_t timestamp) {
  sensor_msgs::msg::BatteryState msg;
  msg.header.stamp = eeros::control::rosTools::convertToRosTime(timestamp);
  msg.present = static_cast<uint8_t>( value );
  self.batteryStatePublisher->publish(msg);
}
