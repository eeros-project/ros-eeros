...
namespace roseeros {
  ...
  public:
    DigOut(std::string id, void* libHandle, std::string device, uint32_t subDeviceNumber,
  ...
  private:
    void (*setFunction) (const DigOut& self, const bool, const uint64_t timestamp);

    // set functions for ROS
    // /////////////////////
    static void sensorMsgsBatteryStatePresent(const DigOut& self, const bool value, const uint64_t timestamp);
